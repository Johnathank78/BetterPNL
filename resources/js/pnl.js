...
function displayNewData(walletData){
  if(API['API'] == "noData" || walletData == false || !fullyLoaded){return};
...
const formattedPnl = params.isPercentage ? fixNumber(Math.abs(pnlNumber / coin.buy_value) * 100, 2) : fixNumber(Math.abs(pnlNumber), 2) ;
...
async function pnl(){
$('.simulator').append($('<span class="versionNB noselect" style="position: absolute; top: 13px; right: 10px; font-size: 14px; opacity: .5; color: white;">v3.7</span>'))
